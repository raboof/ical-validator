...
public class IcalendarValidationService {
	private static final ValidationRuleInfo VALID_CHARSET = new ValidationRuleInfo("4.1.4");

	private static final Log LOG = LogFactory.getLog(IcalendarValidationService.class);
	...
	public static WebMarkupContainer getValidationResult(String id, InputStream calendarStream,
		...
		{
			validationResultHandler.onValidationResult(new ValidationResult(null, "Charset was [{0}] instead of utf-8 as recommended", new Object[] { charSet }, VALID_CHARSET));
		}
		...
		return(new ValidationResultOverviewPanel(id, charSet, (List<ValidationResult>) validationResultHandler
				.getResults()));

	}

	static String getCalendarString(InputStream calendarStream,
			String charSet) {
		try {
	...
	private static void checkCharset(byte[] bytes, String charSet, int offset, int length, int lineno)
	{
		...
		{
			try {
				ValidationResultHandler.get().onValidationResult(new ValidationResult(null, "Input contains non-[{0}] characters on line {1}", new Object[] { charSet, lineno }, VALID_CHARSET));
	...
	private static String fixNewlines(String calendarString) throws ValidationException {
		...
		{
			ValidationResultHandler.get().onValidationResult(new ValidationResult(null, "CRLF should be used for newlines", VALID_CRLF_NEWLINES));
		}
